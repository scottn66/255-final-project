...
|---|---|---|
| **Unoriginal** | Wavelength-conditioned, variable-channel HSI transformers already exist with code and weights: DOFA (2024), HyperFree (CVPR 2025), CARL (DKFZ/Siemens, Apr 2025), Laprade et al. (Mar 2025), HyperSL (TGRS 2025), SpecAware (ISPRS 2026), LESSViT (May 2026), HyperVision (May 2026, checkpoints Jul 2026). LESSViT's *stated problem* is exactly "spectral configuration shift" and its solution is channel-agnostic tied embedding plus wavelength-aware rotary PE. SSFT-Wave re-implements that idea at small scale. | LESSViT 2605.18541; HyperVision 2605.17286; CARL 2504.19223; HyperFree 2503.21841 |
| **Toy** | SSFT itself (CVPR 2026 PBVS workshop) was validated on HSI-Benchmark (ranked first with <2% of the prior leader's parameters) and on SpectralEarth. A reimplementation tested only on Indian Pines / Pavia is strictly weaker evidence than the paper it copies. | SSFT 2604.15828, CVPRW 2026 |
| **Toy (benchmark)** | "Ten Architectures, One Error" (Sep 2026): under the customary random split ~47.7% of test pixels sit within one pixel of a training pixel; a leakage-free protocol drops Macro-F1 by 0.147 on average across ten architecture families and reshuffles rankings by up to five places. Crucially, all ten models misclassify largely the *same* pixels: the residual error is spectral ambiguity in the data, not something a new architecture fixes. A block split is only leakage-free if the separation radius is at least the receptive field, which must be reported. | arXiv 2609.01786 |
...
| **Wrong baseline** | LESSViT's code compares against DOFA, DINOv3, SpecViT, SpatSigma, ChannelViT and HyperFree. A 2026 reviewer will not accept "vanilla ViT with zero-filled bands" as the only comparison for a robustness-under-band-shift claim. | LESSViT repo |
...
|---|---|---|
| **Compact HSI models on real industrial / robotic sorting data with edge deployment** | SpectralWaste (IROS 2024) state of the art is BCAF (Mar 2026): dual Swin backbones, 76.4% mIoU at 31 img/s. EWSegNet (Jun 2026) uses a 23.3M-parameter encoder. Neither reports edge deployment. WoodVIT and HSI-AgriFoodAnomaly (both Scientific Data 2026) ship only 4-layer CNN patch baselines with no transformer, no cross-sensor test, no latency numbers. | HSI + transformers + robotics + NPU |
...
- **Datasets (all public, all Colab-scale).** MWIR-4-Plastic (Zenodo 22142947; 4 co-registered sensors, held-out cube per class). SpectralWaste (Hugging Face `ferpb/spectralwaste-segmentation`, CC BY-NC 4.0; 224 SWIR bands 900–1700 nm; 852 labeled + 6,801 unlabeled; published baselines 64.5–76.4% mIoU). Optional third: HyperPlastic (VIS-NIR + NIR-SWIR, 232 bands) or HSI-AgriFoodAnomaly (300 VNIR bands, foreign-object masks).
...
- **Reuse.** Everything in `src/` (attention, PE, spectral stub, band-mask API). Indian Pines / Pavia drop out or remain as a one-line sanity check.
...
- Benchmarking Foundation Models for Hyperspectral Image Classification: Cereal Crop Type Mapping (WHISPERS 2025). https://arxiv.org/abs/2510.11576
...
- SpectralWaste Dataset: Multimodal Data for Waste Sorting Automation (IROS 2024). https://arxiv.org/abs/2403.18033 · data https://huggingface.co/datasets/ferpb/spectralwaste-segmentation
