...
|---|---|---|---|
| SpectralWaste labeled (HF WebDataset) | 25 GB, 27 shards; Hub viewer is broken so pull shards directly with the `webdataset` library | None beyond TIFF decode; cubes are (224, 256, 256) uint16 | Fetch the 224 FX17 band centers from the dataset repo; split by `sequence_id`, never by frame |
| MWIR-4-Plastic (Zenodo 22142947) | Size unverified, likely 25–30 GB as ENVI cubes | Loader exists; reconcile `semantic_mask.png` vs `semantic_mask2.png` filename mismatch; RGB not aligned | Start the download in week 1 as a background task |
